...
1.  Automatic SSL for WP-Login.php and Administration area 
2.  SSL security for entire logged in session -  Even when browsing blog while logged in.  
3.  PHP Code only for better compatibility - No JavaScript code.  
4.  Easy installation - Just install the plugin on a site that has and SSL certificate and it works.  
4.  No changes to WP-Config.php required.  
5.  Compatible with WordPress 3.3.1 and higher.  
6.  Reverts back to HTTP when logged off.  
7.  Converts URLs to lowercase for better SEO.   
